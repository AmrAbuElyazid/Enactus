...
app.controller('TeacherFriendshipController', ['$scope', '$http', '$mdDialog', function ($scope, $http, $mdDialog) {
	$scope.friends = null;
	...
	$scope.getPendingRequests = function () {
		$http({
			...
			response.data.friends.map(function (friend) {
				if (friend.teacherHasPendingFriendRequest == true) {
	...
	$scope.showDate = function (id) {
        $http({
            ...
            $mdDialog.show({
            	...
            	$scope.sendFriendRequestToStudent(id);
            });
    ...
    $scope.acceptFriendRequest = function (id, request) {
    	$http({
    		method: 'POST',
    		url: '/teacher/accept/student/' + id,
    	}).then(function success(response) {
    		var newItems = $scope.pendingReqests.filter(function (i){
				return request.id !== i.student.id;
			});
